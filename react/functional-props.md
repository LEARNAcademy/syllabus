...
class ColorOptions extends Component {
  render() {
    return (
      <>
        // setting up the button that will eventually change the color of the
        box
        <button></button>
      </>
    );
...
```javascript
<ColorOptions value={value} key={index} changeColor={this.changeColor} />
```
