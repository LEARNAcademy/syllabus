...
Create the describe and it blocks for pertaining to the that feature only. If there are more than one it block, put in pending in the blocks.
Put in expects in one it block to assert how the objects/the system should behave.
Run the tests and see that they fail.
Implement just enough of the software to make the tests pass.
...
Resources
Read this blog entry introduction to RSpec from Treehouse for a slightly more formal grounding in the basics.

Relish RSpec official documentation.
